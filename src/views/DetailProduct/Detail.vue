  ...
  <div class="container">
    <div class="row detail-container">
      <div class="col-md-5 image-container">
        <img :src="list && list.image"
          alt="" />
      </div>
      <div class="col-md-7 right-detail">
        <h2 class="product-title">{{ list.title }}</h2>
        <div class="descript">{{ list.descript }}</div>
        <div class="price"><span class="twd">TWD$</span><span class="count">{{ list.price }}</span></div>
        <div class="buy-count">
          <div class="buy-i"
            @click="handleMin"><i class="el-icon-arrow-down"></i></div>
        ...
        <div class="buy-now"><button type="button"
            class="btn"
            @click="setItemToBuycart">加入購物車</button>
          <button class="btn">
            <router-link tag="span"
              to="/BuyCart">前往結帳</router-link>
          </button>
        </div>
      </div>
    </div>
